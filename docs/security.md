...
- Every `/mcp`, `/sse`, and API request must require a connector token. Prefer `Authorization: Bearer <token>` when the client supports headers; use `?token=<token>` only for web connector UIs that can only accept a URL.
